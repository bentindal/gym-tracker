  ...
  describe 'associations' do
    ...
    describe '#following' do
      it 'returns confirmed friends the user is following' do
        ...
        Friend.create(user: user.id, follows: other_user.id, confirmed: true)
        expect(user.following.count).to eq(1)
      end

      it 'does not return unconfirmed friendships' do
        other_user = create(:user)
        Friend.create(user: user.id, follows: other_user.id, confirmed: false)
    ...
    describe '#followers' do
      it 'returns confirmed friends following the user' do
        other_user = create(:user)
        Friend.create(user: other_user.id, follows: user.id, confirmed: true)
        expect(user.followers.count).to eq(1)
      end

      it 'does not return unconfirmed followers' do
        other_user = create(:user)
        Friend.create(user: other_user.id, follows: user.id, confirmed: false)
  ...
  describe '#last_exercise' do
    context 'when user has exercises' do
      it 'returns the exercise of the last set' do
        exercise = create(:exercise, user_id: user.id)
        Allset.create(exercise_id: exercise.id, user_id: user.id, weight: 100, repetitions: 8)
  ...
  describe '#manually_end_workout' do
    context 'when user has unassigned sets' do
      it 'creates a new workout and assigns the sets' do
        exercise = create(:exercise, user_id: user.id)
        set1 = Allset.create(exercise_id: exercise.id, user_id: user.id, weight: 100, repetitions: 10,
                             belongs_to_workout: nil, created_at: 1.hour.ago)
        set2 = Allset.create(exercise_id: exercise.id, user_id: user.id, weight: 110, repetitions: 8,
                             belongs_to_workout: nil, created_at: 30.minutes.ago)
