...
task fix_workouts: :environment do
  puts "[#{Time.zone.now}] Checking for empty fields in workouts..."
  Workout.find_each do |workout|
    if ['', nil, ' ',
        ''].include?(workout.title) ||
       workout.exercises_used == ' ' ||
       workout.exercises_used.nil? ||
       workout.sets_completed == ' ' ||
       workout.sets_completed.nil?

      puts "Workout #{workout.id} has a missing field. Fixing..."
      workout.title = Allset.where(belongs_to_workout: workout.id)
                            .group_by(&:exercise)
                            .keys
                            .map(&:group)
                            .uniq
