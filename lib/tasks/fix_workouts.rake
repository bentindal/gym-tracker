...
task :fix_workouts => :environment do
    ...
    Workout.all.each do |workout|
        if workout.title == nil || workout.title == "" || workout.exercises_used == " " || workout.exercises_used == nil || workout.sets_completed == " " || workout.sets_completed == nil
            puts "Workout #{workout.id} has a missing field. Fixing..."
            workout.title = Allset.where(belongs_to_workout: workout.id).group_by(&:exercise).keys.map(&:group).uniq.join(", ").reverse.sub(",", "& ").reverse
